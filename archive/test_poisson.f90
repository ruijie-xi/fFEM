...
#include <petsc/finclude/petscpcdef.h>
    ...
    call CreateSolver(ksp, A, A, ierr)
    call timer_end(t_test)
    ...
    call AssembleVectorElement(rhs_func, [1d0,1d0], Th, Vh, 0, assemble_info, Gauss_type, ierr, b)
    deallocate(assemble_info)
    allocate(assemble_info(2,3))
    assemble_info(1,:) = (/1, 1, DERIV_NONE/)
    assemble_info(2,:) = (/2, 2, DERIV_NONE/)
    call AssembleVectorBdry(2, g_func, [1d0,1d0], Th, Vh, 0, assemble_info, Gauss_type_bdry, ierr, b)
    call timer_end(t_test)
