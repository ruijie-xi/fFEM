...
#include <petsc/finclude/petscpcdef.h>
    ...
    call CreateMat(A, Vh%N_DOF + Ph%N_DOF, Vh%N_DOF + Ph%N_DOF, ierr)
    call CreateMat(P, Vh%N_DOF + Ph%N_DOF, Vh%N_DOF + Ph%N_DOF, ierr)
    call CreateVec(b, Vh%N_DOF + Ph%N_DOF, ierr)
    call CreateVec(x, Vh%N_DOF + Ph%N_DOF, ierr)
    call CreateSolver(ksp, A, P, ierr)
    call timer_end(t_test)
    ...
    call AssembleMatrixElement(one_func, [1d0,1d0,1d0,1d0], Th, Vh, 0, Vh, 0, assemble_info, Gauss_type, ierr, A)
    call AssembleMatrixElement(one_func, [1d0,1d0,1d0,1d0], Th, Vh, 0, Vh, 0, assemble_info, Gauss_type, ierr, P)
    ...
    call AssembleMatrixElement(one_func, [-1d0,-1d0], Th, Ph, Vh%N_DOF, Vh, 0, assemble_info, Gauss_type, ierr, A)
    deallocate(assemble_info)
    allocate(assemble_info(2,5))
    assemble_info(1,:) = (/1, 1, DERIV_DX, 1, DERIV_NONE/)
    assemble_info(2,:) = (/1, 2, DERIV_DY, 1, DERIV_NONE/)
    call AssembleMatrixElement(one_func, [-1d0,-1d0], Th, Vh, 0, Ph, Vh%N_DOF, assemble_info, Gauss_type, ierr, A)
    deallocate(assemble_info)
    allocate(assemble_info(1,5))
    assemble_info(1,:) = (/1, 1, DERIV_NONE, 1, DERIV_NONE/)
    call AssembleMatrixElement(one_func, [1d-6], Th, Ph, Vh%N_DOF, Ph, Vh%N_DOF, assemble_info, Gauss_type, ierr, A)
    call AssembleMatrixElement(one_func, [1d0], Th, Ph, Vh%N_DOF, Ph, Vh%N_DOF, assemble_info, Gauss_type, ierr, P)
    deallocate(assemble_info)

    allocate(assemble_info(2,3))
    assemble_info(1,:) = (/1, 1, DERIV_NONE/)
    assemble_info(2,:) = (/2, 2, DERIV_NONE/)
    call AssembleVectorElement(f_func, [1d0,1d0], Th, Vh, 0, assemble_info, Gauss_type, ierr, b)
    deallocate(assemble_info)
    allocate(assemble_info(1,3))
    assemble_info(1,:) = (/1, 1, DERIV_NONE/)
    call AssembleVectorElement(g_func, [1d0], Th, Ph, Vh%N_DOF, assemble_info, Gauss_type, ierr, b)
    call timer_end(t_test)
    ...
    call timer_start()
    call DirichletBC(A, P, b, Th, Vh, u_func, ierr)
    call timer_end(t_test)
    ...
    call PlotFunction(xx_v(1:Vh%N_DOF), Th, Vh, "output/u.vtk")
    write(*,*) "Solution written to output/u.vtk"

    ! compute error
    call ComputeError(u_func, xx_v(1:Vh%N_DOF), Th, Vh, NORM_L2, Gauss_type, L2error)
    write(*,*) "L2 error of u = ", L2error
    call ComputeError(u_func, xx_v(1:Vh%N_DOF), Th, Vh, NORM_H1, Gauss_type, H1error)
    write(*,*) "H1 error of u = ", H1error
    call ComputeError(p_func, xx_v(Vh%N_DOF+1:Vh%N_DOF+Ph%N_DOF), Th, Ph, NORM_L2, Gauss_type, L2error)
...
subroutine DirichletBC(A, P, b, Th, Vh, bndy_func, ierr)
    use settings
    use solver
    use fe
    Mat :: A
    ...
    do i_bdry = 1, Th%N_bdryedge
        ...
        if (Th%BdryMarker(i_bdry) == 0) then
            ...
            do i_dof = 1,size(dof_index)
                ...
                call MatSetValues(P, 1, ind, 1, ind, x, ADD_VALUES, ierr)
