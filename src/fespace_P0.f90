...
subroutine fespaceInit_P0(Vh,Th,dim)
...
end subroutine fespaceInit_P0

subroutine ComputeDof_P0(result,fun,Th, Vh, i_dof)
    type(mesh2D) :: Th
    type(fespace) :: Vh
    procedure(func) :: fun
    ...
    result = 0d0
    call getGaussAnyElement(Th%NodeCoord(:,Th%ElemNodeConn(:,i_elem)), TrianglePt9, x, w)
...
end subroutine ComputeDof_P0

subroutine BasisLocalP0(refpts, Th, Vh, i_elem, deriv_type, result)
    ...
    case (DERIV_DY)
        result = 0d0
    case default
        print *, 'BasisLocalP0: Unknown derivative type'
        stop
...
end subroutine BasisLocalP0

end module fespace_P0