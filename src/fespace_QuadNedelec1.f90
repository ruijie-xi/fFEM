...
subroutine fespaceInit_QuadNedelec1(Vh,Th,dim)
    ...
    Vh%N_local_basis = 4
    call assert(dim.eq.2, "fespaceInit_QuadNedelec1: dim must be 2")
    Vh%dim = dim
    Vh%basis_type = DOF_QuadNedelec1
    Vh%N_DOF = Th%N_edge
    Vh%isStack = 0
    allocate(Vh%ElemDOF(Vh%N_local_basis, Th%N_elem))
    Vh%ElemDOF = abs(Th%ElemEdgeConn)
    
end subroutine fespaceInit_QuadNedelec1

subroutine ComputeDof_QuadNedelec1(result,fun,Th,i_dof)
    type(mesh2D) :: Th
    procedure(func) :: fun
    ...
    call getEdgeTangent(Th, i_edge, tangent)
    call getEdgeLength(Th, i_edge, length)

    result = sum(w*val(1,:)*tangent(1) + w*val(2,:)*tangent(2))/length
    

end subroutine ComputeDof_QuadNedelec1

subroutine getEdgeDofIndex_QuadNedelec1(Th, Vh, i_edge, dof_index)
    type(mesh2D), intent(in) :: Th
...
end subroutine getEdgeDofIndex_QuadNedelec1


subroutine BasisReferenceQuadNedelec1(refpts, deriv_type, result)
    ...
    A1(:, 1) = (/ 1d0, 0d0, 0d0, 0d0 /)
    A1(:, 2) = (/ 0d0, 0d0, 0d0, 0d0 /)
    A1(:, 3) = (/ -1d0, 0d0, -1d0, 0d0 /)

    A2(:, 1) = (/ 0d0, 0d0, 0d0, -1d0 /)
    A2(:, 2) = (/ 0d0, 1d0, 0d0, 1d0 /)
    A2(:, 3) = (/ 0d0, 0d0, 0d0, 0d0 /)
    ...
    case (DERIV_DY)
        ...
        b(3,:) = 1d0
    case default
        print *, 'BasisQuadNedelec1: Unknown derivative type'
        stop
    end select
...
end subroutine BasisReferenceQuadNedelec1

subroutine BasisLocalQuadNedelec1(refpts, Th, Vh, i_elem, deriv_type, result)
    real(8), intent(in), dimension(:,:) :: refpts
    type(mesh2D), intent(in) :: Th
    type(fespace), intent(in) :: Vh
    ...
    integer :: num_pts, i_dim, num_basis, i
    ...
    case (DERIV_NONE)
        ...
        call BasisReferenceQuadNedelec1(refpts, DERIV_NONE, basis_ref)

        ! Piola transform
        basis(1,:,:) = spread(J22,1,num_basis)*basis_ref(1,:,:)/spread(J,1,num_basis) &
        - spread(J21,1,num_basis)*basis_ref(2,:,:)/spread(J,1,num_basis)
        basis(2,:,:) = -spread(J12,1,num_basis)*basis_ref(1,:,:)/spread(J,1,num_basis) &
        + spread(J11,1,num_basis)*basis_ref(2,:,:)/spread(J,1,num_basis)
    ...
    case (DERIV_DX)
        ...
        call BasisReferenceQuadNedelec1(refpts, DERIV_DX, basis_ref_dxh)
        call BasisReferenceQuadNedelec1(refpts, DERIV_DY, basis_ref_dyh)
        ...
        ! Piola transform
        basis_dx(1,:,:) = spread(J22,1,num_basis)*basis_ref_dx(1,:,:)/spread(J,1,num_basis) &
        - spread(J21,1,num_basis)*basis_ref_dx(2,:,:)/spread(J,1,num_basis)
        basis_dx(2,:,:) = -spread(J12,1,num_basis)*basis_ref_dx(1,:,:)/spread(J,1,num_basis) &
        + spread(J11,1,num_basis)*basis_ref_dx(2,:,:)/spread(J,1,num_basis)
    ...
    case (DERIV_DY)
        ...
        ! Piola transform
        basis_dy(1,:,:) = spread(J22,1,num_basis)*basis_ref_dy(1,:,:)/spread(J,1,num_basis) &
        - spread(J21,1,num_basis)*basis_ref_dy(2,:,:)/spread(J,1,num_basis)
        basis_dy(2,:,:) = -spread(J12,1,num_basis)*basis_ref_dy(1,:,:)/spread(J,1,num_basis) &
        + spread(J11,1,num_basis)*basis_ref_dy(2,:,:)/spread(J,1,num_basis)
    ...
    case default
        print *, 'BasisLocalQuadNedelec1: Unknown derivative type'
        stop

    end select
    
end subroutine BasisLocalQuadNedelec1
...
end module fespace_QuadNedelec1