program test_matvec
    ...
    ! matrix:
    ! [1, 0, 0, 2, 0, 0]   [0, 3, 0, 0, 4, 1]
    ! [0, 3, 0, 0, 4, 0]   [0, 0, 0, 0, 0, 0]
    ! [0, 0, 5, 0, 0, 1] + [0, 0, 6, 0, 0, 3]
    ! [6, 0, 0, 7, 0, 2]   [0, 0, 0, 0, 9, 0]
    ! [0, 8, 0, 0, 9, 3]   [0, 0, 0, 0, 0, 0]
    ...
    call MatrixTripletInit(A1, 5, 6, 12)
    call MatrixTripletInit(A2, 5, 6, 6)

    A1%val = [1, 2, 3, 4, 5, 6, 7, 8, 9, 1, 2, 3]
    A1%row_idx = [1, 1, 2, 2, 3, 4, 4, 5, 5, 3, 4, 5]
    A1%col_idx = [1, 4, 2, 5, 3, 1, 4, 2, 5, 6, 6, 6]

    A2%val = [3, 4, 6, 9, 1, 3]
    A2%row_idx = [1, 1, 3, 4, 1, 3]
    A2%col_idx = [2, 5, 3, 5, 6, 6]
